        ...
        for (call, MulticallResult { success, returnData }) in self.calls.iter().zip(iter) {
            ...
            let result = if !success {
                ...
                if let Err(err) = decoded {
                    ...
                    if !call.allow_failure {
                        println!("BUG: Could not decode address {}: {:?}", call.target, err);
                        return Err(err);
                    }
                    Err(Bytes::default())
                } else {
...
mod tests {
    ...
    use super::*;
    use crate::addresses::{MULTICALL_ADDRESS_DEFAULT_CHAINS, MULTICALL_DEFAULT_ADDRESS};
