...
//!
//! This file includes code from the Reth project.
...
impl CliRunner {
        ...
        E: Send + Sync + From<std::io::Error> + From<reth_tasks::PanickedTaskError> + 'static,
    {
        ...
        if command_res.is_err() {
            error!(target: "hyve::cli", "shutting down due to error");
        } else {
            debug!(target: "hyve::cli", "shutting down gracefully");
        ...
        let _ = rx.recv_timeout(Duration::from_secs(5)).inspect_err(|err| {
            debug!(target: "hyve::cli", %err, "tokio runtime shutdown timed out");
        });
    ...
    E: Send + Sync + 'static + From<std::io::Error>,
{
    ...
    #[cfg(unix)]
    {
        ...
        tokio::select! {
            _ = ctrl_c => {
                trace!(target: "hyve::cli", "Received ctrl-c");
            },
            _ = sigterm => {
                trace!(target: "hyve::cli", "Received SIGTERM");
            },
            res = fut => res?,
        }
    }
