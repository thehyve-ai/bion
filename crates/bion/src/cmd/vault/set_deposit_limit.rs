...
pub struct SetDepositLimitCommand {
    ...
    #[arg(value_name = "DEPOSIT_LIMIT", help = "The deposit limit of the vault.")]
    deposit_limit: U256,
...
impl SetDepositLimitCommand {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        ...
        let arg = SendTxArgs {
            to: Some(to),
            sig: Some("setDepositLimit(uint256 limit)".to_string()),
            args: vec![deposit_limit.to_string()],
            cast_async: false,
            confirmations,
            command: None,
            unlocked,
        ...
        if let Ok(..) = arg.run().await {
            print_success_message("✅ Successfully set vault deposit limit.");
        } else {
            print_error_message("❌ Failed to set vault deposit limit, please try again.");
        }
        Ok(())
    }
}
