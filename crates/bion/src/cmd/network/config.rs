...
use super::consts::NETWORK_DIRECTORY;
...
impl NetworkConfig {
    ...
    pub fn set_signing_method(&mut self, dirs: &DirsCliArgs) -> eyre::Result<()> {
        ...
        let network_config_dir = data_dir.join(format!("{}/{}", NETWORK_DIRECTORY, self.address));
        ...
        match selection {
            0 => {
                ...
                self.keystore_file = Some(network_config_dir);
            ...
            2 => {
                ...
                self.signing_method = Some(SigningMethod::Keystore);
                self.keystore_file = Some(network_config_dir.clone());
                self.password_enabled = true;
